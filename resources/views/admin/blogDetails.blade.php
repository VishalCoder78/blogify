            ...
            <form class="mb-6" method="POST" action="{{ route('comment') }}">
                ...
                <div
                    ...
                    <div class="w-full">
                        ...
                        <input type="text" class="hidden" value="{{ Auth::guard('admin')->user()->name }}"
                            name="user_name">
                        <input type="text" class="hidden" value="admin" name="auth_type">
                    ...
                    <article class="p-6 mb-6 text-base bg-gray-900 rounded-lg dark:bg-gray-900">
                        <footer class="flex justify-between items-center my-4">
                            <div class="flex items-center">
                                ...
                                @endif
                                @if ($comment->auth_type == 'admin')
                                    <p class="inline-flex items-center mr-3 text-sm text-gray-400 dark:text-white"><img
                                            class="mr-2 w-6 h-6 rounded-full"
                                            src="https://ui-avatars.com/api/?name={{ $comment->user_name }}/&background=random "
                                            alt="Michael Gough">{{ $comment->user_name }}</p> <span
                                        class="items-center rounded-full bg-gray-700 px-1.5 py-0.5 text-xs font-medium text-gray-100 border border-gray-200">Admin</span>
                                    <p class="text-sm text-gray-400 dark:text-gray-400">
                                @endif
                                <time class="mx-2">{{ $comment->created_at->format('d/m/Y') }}</time></p>
                            </div>
